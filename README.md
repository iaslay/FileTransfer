...
├── images/                   # 项目截图（运行界面、编译过程等）
├── build_windows/            # Windows 编译输出目录（可下载）
...
如需直接使用编译好的 Windows 版本，可从以下链接下载 `build_windows` 目录（内含 `FileTransfer.exe` 及所需 DLL）：

- **Google Drive：** [build_windows 下载](https://drive.google.com/drive/folders/1vMDc7sCTDj7M1bK51Uy8Xw0vAD38welB?usp=drive_link)

> 该目录包含编译好的 `FileTransfer.exe` 及运行时依赖库，下载后可直接运行，无需安装 Qt 开发环境。
树莓派的需要自行编译，不同版本差异较多，不好直接上传可执行文件和编译好的qt
